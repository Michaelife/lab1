...
if(isset($_POST['drug_name']))
{
	...
	$sql=mysqli_query($conn,"INSERT INTO tempPrescri(customer_id,customer_name,age,sex,postal_address,phone,drug_name,strength,dose,quantity)
...
$getDetails=mysqli_query($conn,"SELECT invoice,drug,cost,quantity FROM invoice_details WHERE invoice='{$invoice}'");
$getQuantity=mysqli_query($conn,"SELECT quantity FROM invoice_details WHERE invoice_id='{$invoice}'");
