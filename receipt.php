...
$getCid=mysqli_query($conn,"SELECT customer_id FROM prescription WHERE invoice_id='{$invNo}' ");
$getName=mysqli_query($conn,"SELECT customer_name FROM invoice WHERE invoice_id='{$invNo}' ");
$details1=mysqli_fetch_array($getName);
$details=mysqli_fetch_array($getCid);

	
	$sqlP=mysqli_query($conn,"INSERT INTO receipts(reciptNo,customer_id,total,payType,serialno,served_by)
